...
import {
  getUserChannel, subscribeToChannel,
  getChannelVideos, getChannelPlaylists, getChannelTweets,
...
export default function Channel() {
  ...
  const handleSubscribe = async () => {
    ...
    try {
      ...
      setSubscribed(prev => { setSubCount(c => prev ? c - 1 : c + 1); return !prev; });
    } catch {}
    setSubLoading(false);
  };
  ...
  const searchResults = searchQuery.trim()
    ? allPublic.filter(v => (v.title || "").toLowerCase().includes(searchQuery.toLowerCase()))
    : [];
  ...
  const switchTab = (key: Tab) => { setActiveTab(key); setSearchOpen(false); setSearchQuery(""); };

  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-100">

      {/* Hidden file inputs */}
      <input ref={avatarInput} id="ch-avatar-file-input" type="file" accept="image/*" className="hidden" onChange={handleAvatarUpload} />
      <input ref={coverInput}  id="ch-cover-file-input"  type="file" accept="image/*" className="hidden" onChange={handleCoverUpload}  />

      {/* ── Banner ──────────────────────────────────────────────────────────── */}
      <div className="relative w-full h-[clamp(120px,22vw,280px)] bg-gradient-to-br from-violet-950 to-zinc-950 overflow-hidden group/banner">
        {channel.coverimage && (
          <img src={channel.coverimage} alt="banner" className="w-full h-full object-cover" />
        )}
        {canEdit && (
          <BannerEditButton uploading={uploadingCover} onClick={() => coverInput.current?.click()} />
        )}
        {uploadSuccess === "cover" && <UploadToast message="Banner updated!" />}
      </div>

      {/* ── Header ──────────────────────────────────────────────────────────── */}
      <div className="bg-zinc-900/80 border-b border-white/[0.06] backdrop-blur-sm">
        <div className="max-w-[1300px] mx-auto px-10 pt-6 pb-0 flex items-start gap-6">
          ...
          <div className="relative shrink-0 group/avatar">
            <Avatar className="size-24 border-[3px] border-violet-500/50 shadow-lg shadow-violet-900/30 ring-2 ring-zinc-900">
              ...
              <AvatarFallback className="text-2xl font-bold bg-violet-900 text-violet-200">
                {channel.name.charAt(0).toUpperCase()}
              </AvatarFallback>
            </Avatar>
            {canEdit && (
              <AvatarEditButton uploading={uploadingAvatar} onClick={() => avatarInput.current?.click()} />
            )}
          ...
          <div className="flex-1 min-w-0 pb-5">
            <h1 className="text-2xl sm:text-3xl font-bold tracking-tight mb-1.5 bg-gradient-to-r from-zinc-100 to-violet-200 bg-clip-text text-transparent">
              {channel.name}
            </h1>
            ...
            {channel.bio && (
              <p className="text-sm text-zinc-400 leading-relaxed max-w-2xl mb-3">
                ...
                {channel.bio.length > 120 && (
                  <button onClick={() => setShowFullBio(p => !p)}
                    className="text-violet-400 hover:text-violet-300 ml-1 text-sm font-medium transition-colors">
        ...
        <div className="max-w-[1300px] mx-auto px-8 flex items-center border-t border-white/[0.05]">
            ...
            {TABS.map(tab => (
              <button
                ...
                className={`px-5 py-3.5 text-sm font-medium whitespace-nowrap border-b-2 transition-all ${
                  activeTab === tab.key
                    ? "border-violet-500 text-zinc-50 font-semibold"
          ...
          <button
            id="ch-search-toggle"
            onClick={() => { setSearchOpen(p => !p); setTimeout(() => searchRef.current?.focus(), 100); }}
        ...
        {searchOpen && (
          <div className="max-w-[1300px] mx-auto px-10 py-3 flex items-center gap-3 border-t border-white/[0.05]">
            ...
            />
            {searchQuery && (
              <button onClick={() => setSearchQuery("")} className="text-zinc-500 hover:text-zinc-200 text-sm">✕</button>
            )}
          </div>
        )}
      </div>
      ...
      <div className="max-w-[1300px] mx-auto px-10 py-8 space-y-12">
          ...
          <section>
            ...
            {searchResults.length === 0
              ...
              : <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                  {searchResults.map(v => <VideoCard key={v.videoId} video={v} onClick={() => navigate(`/video/${v.videoId}`)} />)}
                </div>
            }
          </section>
        )}
        ...
        {!searchOpen && activeTab === "home" && (
          <>
            <section>
              ...
              {dataLoading ? <VideoGridSkeleton /> : regularVids.length === 0
                ? <EmptyState icon="🎬" title="No videos yet" sub="This channel hasn't posted any videos" />
                : <ScrollRow>{regularVids.slice(0, 12).map(v =>
            ...
            {regularVids.length > 0 && (
              <section>
                ...
                <ScrollRow>{regularVids.slice(0, 8).map(v =>
                  <div key={v.videoId} className="w-[280px] shrink-0">
                    <VideoCard video={v} onClick={() => navigate(`/video/${v.videoId}`)} />
                  </div>
                )}</ScrollRow>
            ...
            {shorts.length > 0 && (
              <section>
                ...
                <ScrollRow>{shorts.slice(0, 8).map(v =>
        ...
        {!searchOpen && activeTab === "videos" && (
          <section>
            ...
            {dataLoading ? <VideoGridSkeleton /> : sortedVideos.length === 0
              ? <EmptyState icon="🎬" title="No videos yet" sub="This channel hasn't posted any videos" />
              : <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-x-4 gap-y-6">
                  {sortedVideos.map(v => <VideoCard key={v.videoId} video={v} onClick={() => navigate(`/video/${v.videoId}`)} />)}
                </div>
            }
          </section>
        )}
        ...
        {!searchOpen && activeTab === "shorts" && (
          <section>
            {dataLoading ? <VideoGridSkeleton /> : shorts.length === 0
            ...
            {dataLoading
              ...
              : tweets.length === 0
                ...
                : <div className="space-y-4">
                    {tweets.map(t => <PostCard key={t._id} tweet={t} channel={channel} />)}
                  </div>
            }
          </section>
        )}
