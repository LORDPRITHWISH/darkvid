    ...
    <div className="fixed left-0 top-0 h-screen w-14 mt-8 flex flex-col items-center py-4 z-30">
      ...
      <div className="flex flex-col gap-8 text-gray-400 mt-12 ml-4">
        {links.map(({ icon: Icon, label, link }) => (
          <NavLink className="hover:text-white cursor-pointer flex-col flex space-y-2 items-center justify-center text-gray-400" to={link}>
            <Icon key={label} size={20} aria-label={label} className="mx-auto" />
            <span className="text-xs mx-auto">{label}</span>
          </NavLink>
        ))}
      </div>
    </div>
