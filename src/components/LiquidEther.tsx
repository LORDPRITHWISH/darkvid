...
export default function LiquidEther({
  ...
  useEffect(() => {
    ...
    // Replace the entire MouseClass with this corrected version
    class MouseClass {
      mouseMoved = false;
      ...
      setCoords(x: number, y: number) {
        ...
        const ny = -(((y - rect.top) / rect.height) * 2 - 1); // Flip Y axis
      ...
      onMouseLeave() {
        this.isHoverInside = false;
        // Reset mouse position when leaving to prevent stuck forces
        this.coords.set(0, 0);
      ...
      update() {
        ...
        // Prevent large forces when mouse first enters or after long pause
        if (Math.abs(this.diff.x) > 0.5 || Math.abs(this.diff.y) > 0.5) {
          this.diff.set(0, 0);
        }
